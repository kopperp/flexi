...
!  DO i=1,nSpecies
!    Species(i)%Init%InsertedParticle = INT(Species(i)%Init%ParticleEmission * Time)
!  END DO
!!ELSE
  DO i = 1,nSpecies
    DO iInit = Species(i)%StartnumberOfInits, Species(i)%NumberOfInits
...
USE MOD_Timedisc_Vars         , ONLY : dt,t,RKc,currentStage!,time
!#if defined(LSERK) || defined(IMEX) || defined(IMPA)
!#endif
!USE MOD_Timedisc_Vars          ,ONLY: RKc(currentStage),RKc(currentStage)Total
      ...
      IF (mode.NE.2) THEN
        ...
        CASE(1) ! Emission Type: Particles per !!!!!SECOND!!!!!!!! (not per ns)
          IF (Species(i)%Init(iInit)%VirtPreInsert .AND. Species(i)%Init(iInit)%PartDensity.GT.0.) THEN
            PartIns=Species(i)%Init(iInit)%ParticleEmission * dt*RKc(currentStage)  ! emitted particles during time-slab
          ...
          ELSE IF (.NOT.DoPoissonRounding .AND. .NOT.DoTimeDepInflow) THEN
            ...
            ! partns - changed time step!!
            PartIns=Species(i)%Init(iInit)%ParticleEmission * (t + dt) ! total number of emitted particle over 
                                                                                        ! simulation
          ...
          ELSE IF (DoPoissonRounding .AND. .NOT.DoTimeDepInflow) THEN
            ...
            ELSE
              RiseFactor=1.
            EnD IF
            PartIns=Species(i)%Init(iInit)%ParticleEmission * dt*RKc(currentStage) * RiseFactor  ! emitted particles during time-slab
            CALL RANDOM_NUMBER(RandVal1)
          ...
          ELSE ! DoTimeDepInflow
            ...
            PartIns=Species(i)%Init(iInit)%ParticleEmission * dt*RKc(currentStage) * RiseFactor &
                   + Species(i)%Init(iInit)%InsertedParticleMisMatch
            CALL RANDOM_NUMBER(RandVal1)
            NbrOfParticle = INT(PartIns + RandVal1)
          END IF
      ...
      IF(NbrOfParticle.GT.0)THEN
        ...
        ! ELSE
          ...
          DO iPart=1,NbrOfparticle
            ...
            ! partns - belongs to particle_analyze
...
USE MOD_Globals,          ONLY:TwoepsMach!,PI
USE MOD_Particle_Globals,           ONLY:PI
USE MOD_Timedisc_Vars,         ONLY:dt, currentStage!iter,DoDisplayEmissionWarnings, IterDisplayStep, DoDisplayIter
USE MOD_Timedisc_Vars,         ONLY : RKc
...
,'BaseVectors are parallel!')
  ...
  l_ins=dt*RKc(currentStage) * ( v_drift_line + Species(FractNbr)%Init(iInit)%NSigma & !virt. insertion height
...
,' Something is wrong with the Basis of the current SpaceIC!')
    ...
    IF ( TRIM(Species(FractNbr)%Init(iInit)%SpaceIC) .EQ. 'cuboid_vpi' ) THEN
      lrel_ins_BV=dt*RKc(currentStage)*( (/v_drift_BV(1),-v_drift_BV(1),v_drift_BV(2),-v_drift_BV(2)/)+Species(FractNbr)%Init(iInit)%NSigma &
    ...
    ELSE IF ( TRIM(Species(FractNbr)%Init(iInit)%SpaceIC) .EQ. 'orifice' ) THEN !cylinder-orifice
      lrel_ins_BV(1:4)=dt*RKc(currentStage) * ( SQRT(v_drift_BV(1)**2+v_drift_BV(2)**2) + Species(FractNbr)%Init(iInit)%NSigma &
        * SQRT(BoltzmannConst*Species(FractNbr)%Init(iInit)%MWTemperatureIC/Species(FractNbr)%MassIC) ) &
...
,'BaseVectors are parallel!')
      ...
      DO WHILE (i .LE. chunkSize)
         ...
         IF (Species(FractNbr)%Init(iInit)%CalcHeightFromDt) THEN !directly calculated by timestep
           Particle_pos = Particle_pos + lineVector * Species(FractNbr)%Init(iInit)%VeloIC * dt*RKc(currentStage) * RandVal(3)
         ELSE
...
!        CALL CalcVelocity_maxwell_lpn(FractNbr, Vec3D, iInit=iInit)
        ...
        delta_l = dt*RKc(currentStage) * v_line - l_ins * RandVal(3)
        ...
        CASE('freestream')
          ...
          delta_lBV = dt*RKc(currentStage) * v_BV
        ...
        CASE('orifice')
          ...
          DO WHILE ( radius .GT. Species(FractNbr)%Init(iInit)%RadiusIC * (1.+lrel_ins_BV(1)) )
            ...
            radius = SQRT( Particle_pos(1) * Particle_pos(1) + &
                           ...
                           Particle_pos(3) * Particle_pos(3) )
          END DO
          Particle_pos = Particle_pos + dt*RKc(currentStage)*Vec3D - lineVector*(dt*RKc(currentStage)*v_line-delta_l) !get old RandVal(3) for l_ins*R3
...
!        SWRITE(UNIT_StdOut,'(a3,a30,a3,i33,a3,a7,a3)')' | ',TRIM("IMDNumber")       ,' | ', IMDNumber   ,' | ',TRIM("OUTPUT"),' | '
        ...
        DO i=1,chunkSize
          ...
          ELSE IF(io_error<0)THEN
            ...
            ! partns - no transformation (1=2 WTF?)
!          ELSE
...
USE MOD_TimeDisc_Vars          ,ONLY : TEnd,t!, time
...
USE MOD_Timedisc_Vars          ,ONLY: t,RKc!,RKc(currentStage),RKc(currentStage)Total,Time
...
!read*
        ELSE IF(DoPoissonRounding .AND. .NOT.DoTimeDepInflow)THEN
          PartIns = Species(iSpec)%Surfaceflux(iSF)%PartDensity / Species(iSpec)%MacroParticleFactor &
                  * dt*RKc(currentStage) * Species(iSpec)%Surfaceflux(iSF)%SurfFluxSubSideData(iSample,jSample,iSide)%nVFR
          IF (EXP(-PartIns).LE.TINY(PartIns)) THEN
            CALL abort(&
__STAMP__&
,'ERROR in ParticleSurfaceflux: flux is too large for poisson sampling!')
        ...
        ELSE !DoTimeDepInflow
          ...
          PartInsSubSide = INT(Species(iSpec)%Surfaceflux(iSF)%PartDensity / Species(iSpec)%MacroParticleFactor &
                         * dt*RKc(currentStage) * Species(iSpec)%Surfaceflux(iSF)%SurfFluxSubSideData(iSample,jSample,iSide)%nVFR+RandVal1)
...
!#endif
!USE MOD_Particle_Analyze_Vars  ,ONLY: nPartIn,PartEkinIn
!USE MOD_Timedisc_Vars          ,ONLY: RKc(currentStage)
!USE MOD_Particle_Analyze       ,ONLY: CalcEkinPart
!USE MOD_Mesh_Vars              ,ONLY: SideToElem
!USE MOD_Particle_Mesh_Vars     ,ONLY: PartElemToSide
...
!
!        CALL RANDOM_NUMBER(RandVal1)
!        PartInsSubSide = INT(ElemPartDensity / Species(iSpec)%MacroParticleFactor * dt*RKc(currentStage) * nVFR+RandVal1)
!        !-- proceed with calculated to be inserted particles
!        IF (PartInsSubSide.LT.0) THEN
!          CALL abort(&
!__STAMP__&
...
! partns - ignore this part for now
