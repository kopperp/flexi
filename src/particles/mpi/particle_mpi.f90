...
!   IF(.NOT.PDM%ParticleInside(iPart)) CYCLE
!   ElemID=PEM%Element(iPart)
!   IF(ElemID.GT.PP_N)  PartMPIExchange%nPartsSend(PartHaloElemToProc(LOCAL_PROC_ID,ElemID))=             &
...
SideIndex=0
ALLOCATE(ElemIndex(1:PP_N),STAT=ALLOCSTAT)
IF (ALLOCSTAT.NE.0) CALL abort(&
__STAMP__&
...
, ' Error in PartHaloElemToProc')
  END DO ! iVar=1,3
END DO ! iElem=PP_N+1,nTotalElems
DO iSide = 1,nTotalSides
  DO iVar=1,5
